...
CONTAINS
   ...
   SUBROUTINE ssh_wzv_tan( kt , kdum )
      ...
      IF ( .NOT. PRESENT(kdum) ) THEN             ! jump ssh computing
         ...
         DO jk = 1, jpkm1                                 ! Horizontal divergence of barotropic transports
            zhdivtl(:,:) = zhdivtl(:,:) + fse3t(:,:,jk) * hdivn_tl(:,:,jk)
         END DO
      ...
      DO jk = jpkm1, 1, -1
         wn_tl(:,:,jk) = wn_tl(:,:,jk+1) - fse3t_n(:,:,jk) * hdivn_tl(:,:,jk)
      END DO
      !
   ...
   SUBROUTINE ssh_nxt_tan( kt )
      ...
      !IF (.NOT.Agrif_Root())    CALL Agrif_Update_Dyn_tan( kt )
      IF (lwp) WRITE(numout,*) 'key_agrif not available yet'
      CALL abort
#endif
      !
   ...
   SUBROUTINE ssh_wzv_adj( kt , kdum )
      ...
      !                                                ! integrate from the bottom the hor. divergence
      DO jk = 1, jpkm1
         hdivn_ad(:,:,jk  ) = hdivn_ad(:,:,jk  ) - fse3t_n(:,:,jk) * wn_ad(:,:,jk)
      ...
      IF ( .NOT. PRESENT(kdum) ) THEN             ! jump ssh computing
         ...
         ENDIF
         !                                                ! Sea surface elevation time stepping
         sshb_ad(:,:) = sshb_ad(:,:) + ssha_ad(:,:)* tmask(:,:,1)
         emp_ad( :,:) = emp_ad(:,:)  - z2dt * z1_rau0 * ssha_ad(:,:) * tmask(:,:,1)
         emp_b_ad( :,:) = emp_b_ad(:,:) - z2dt * z1_rau0 * ssha_ad(:,:) * tmask(:,:,1)
         zhdivad(:,:) = zhdivad(:,:) - z2dt * tmask(:,:,1) * ssha_ad(:,:)
         ssha_ad(:,:) = 0.0_wp

         DO jk = 1, jpkm1                                 ! Horizontal divergence of barotropic transports
            hdivn_ad(:,:,jk) = hdivn_ad(:,:,jk) + fse3t(:,:,jk) * zhdivad(:,:)
         END DO
         zhdivad(:,:) = 0._wp
...
#if defined key_agrif
      ! Update velocity at AGRIF zoom boundaries
      !IF (.NOT.Agrif_Root())    CALL Agrif_Update_Dyn_adj( kt )
      IF (lwp) WRITE(numout,*) 'key_agrif not available yet'
      CALL abort
#endif
      IF( lk_vvl ) THEN      ! Variable volume levels :   ssh at t-, u-, v, f-points
