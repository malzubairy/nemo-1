   ...
   SUBROUTINE dyn_ldf_bilap_tan( kt )
      ...
      DO jk = 1, jpkm1                                 ! Horizontal slab
         ...
         IF( ln_sco .OR. ln_zps ) THEN   ! s-coordinate or z-coordinate with partial steps
            zuftl(:,:,jk) = rotb_tl(:,:,jk) * fse3f(:,:,jk)
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zlutl(ji,jj,jk) = - ( zuftl(ji,jj,jk) - zuftl(ji,jj-1,jk) ) / ( e2u(ji,jj) * fse3u(ji,jj,jk) )   &
                     &         + ( hdivb_tl(ji+1,jj,jk) - hdivb_tl(ji,jj,jk) ) / e1u(ji,jj)

                  zlvtl(ji,jj,jk) = + ( zuftl(ji,jj,jk) - zuftl(ji-1,jj,jk) ) / ( e1v(ji,jj) * fse3v(ji,jj,jk) )   &
      ...
      DO jk = 1, jpkm1
         ...
         zlutl(:,:,jk) = zlutl(:,:,jk) * fsahmu(:,:,jk)
         zlvtl(:,:,jk) = zlvtl(:,:,jk) * fsahmv(:,:,jk)
         ...
         DO jj = 1, jpjm1
            DO ji = 1, fs_jpim1   ! vector opt.
               zlutl(ji,jj,jk) = e2u(ji,jj) * fse3u(ji,jj,jk) * zlutl(ji,jj,jk)
               zlvtl(ji,jj,jk) = e1v(ji,jj) * fse3v(ji,jj,jk) * zlvtl(ji,jj,jk)
            END DO
         END DO
      ...
      DO jk = 1, jpkm1
         ...
         DO jj = jpjm1, 1, -1
            DO ji = fs_jpim1, 1, -1   ! vector opt.
               zluad(ji,jj,jk) = e2u(ji,jj) * fse3u(ji,jj,jk) * zluad(ji,jj,jk)
               zlvad(ji,jj,jk) = e1v(ji,jj) * fse3v(ji,jj,jk) * zlvad(ji,jj,jk)
            END DO
         END DO
         ! Laplacian curl ( * e3f if s-coordinates or z-coordinate with partial steps)
         DO jj = jpjm1, 1, -1
            DO ji = fs_jpim1, 1, -1   ! vector opt.
               zufad(ji,jj,jk) = fmask(ji,jj,jk) * zufad(ji,jj,jk) * fse3f(ji,jj,jk) / ( e1f(ji,jj)*e2f(ji,jj) )
         ...
         END DO
         ! Multiply by the eddy viscosity coef. (at u- and v-points)
         zluad(:,:,jk) = zluad(:,:,jk) * fsahmu(:,:,jk)
         zlvad(:,:,jk) = zlvad(:,:,jk) * fsahmv(:,:,jk)
      END DO
      ! Boundary conditions on the laplacian  (zlu,zlv)
