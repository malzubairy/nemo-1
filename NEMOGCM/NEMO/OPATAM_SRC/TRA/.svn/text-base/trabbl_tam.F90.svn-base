...
CONTAINS
   ...
   SUBROUTINE tra_bbl_tan( kt )
      ...
      !
      IF( l_bbl )  CALL bbl_tan( kt, nit000, 'TRA' )   !* bbl coef. and transport (only if not already done in trcbbl)
...
# if defined control_param
                  &               + (   ahu_bbl_tl(ji  ,jj  ) * ( zptb(ji+1,jj  ) - zptb(ji  ,jj  ) )    &
                  &                   - ahu_bbl_tl(ji-1,jj  ) * ( zptb(ji  ,jj  ) - zptb(ji-1,jj  ) )    &
                  &                   + ahv_bbl_tl(ji  ,jj  ) * ( zptb(ji  ,jj+1) - zptb(ji  ,jj  ) )    &
                  &                   - ahv_bbl_tl(ji  ,jj-1) * ( zptb(ji  ,jj  ) - zptb(ji  ,jj-1) )   ) * zbtr &
#endif
...
# endif
      ...
      IF( nn_bbl_ldf == 1 ) THEN          !   diffusive bbl   !
         ...
         DO jj = 1, jpjm1                      ! (criteria for non zero flux: grad(rho).grad(h) < 0 )
            DO ji = 1, jpim1
               ...
               ahu_bbl_tl(ji,jj) = ( 0.5 - zsign ) * ahu_bbl_0_tl(ji,jj)             ! masked diffusive flux coeff.
               !
               !                                                ! j-direction
               ...
               ahv_bbl_tl(ji,jj) = ( 0.5 - zsign ) * ahv_bbl_0_tl(ji,jj)
               !
            END DO
         END DO
#else
   ...
   SUBROUTINE tra_bbl_adj( kt )
      ...
      !
      IF( l_bbl )  CALL bbl_adj( kt, nit000, 'TRA' )   !* bbl coef. and transport (only if not already done in trcbbl)
...
#  endif
               ...
               zbtr = r1_e1e2t(ji,jj)  / fse3t(ji,jj,ik)
#  if defined control_param
               ahu_bbl_ad(ji  ,jj  ) = ahu_bbl_ad(ji  ,jj  ) + pta_ad(ji,jj,ik,jn) * ( zptb(ji+1,jj  ) - zptb(ji  ,jj  ) ) * zbtr
               ahu_bbl_ad(ji-1,jj  ) = ahu_bbl_ad(ji-1,jj  ) - pta_ad(ji,jj,ik,jn) * ( zptb(ji  ,jj  ) - zptb(ji-1,jj  ) ) * zbtr
               ahv_bbl_ad(ji  ,jj  ) = ahv_bbl_ad(ji  ,jj  ) + pta_ad(ji,jj,ik,jn) * ( zptb(ji  ,jj+1) - zptb(ji  ,jj  ) ) * zbtr
               ahv_bbl_ad(ji  ,jj-1) = ahv_bbl_ad(ji  ,jj-1) - pta_ad(ji,jj,ik,jn) * ( zptb(ji  ,jj  ) - zptb(ji  ,jj-1) ) * zbtr
#  endif
...
# if defined key_vectopt_loop
         DO jj = 1, 1
            DO ji = jpij-jpi-1, 1, -1   ! vector opt. (forced unrolling)
# else
         DO jj = jpjm1, 1, -1
...
# endif
         ...
         DO jj = 1, jpjm1                      ! (criteria for non zero flux: grad(rho).grad(h) < 0 )
            DO ji = 1, jpim1
               !                                                ! j-direction
                  ...
                  !
               zsign          = SIGN(  0.5, -zgdrho * REAL( mgrhv(ji,jj) )  )     ! sign of ( j-gradient * j-slope )
               ahv_bbl_0_ad(ji,jj) = ( 0.5 - zsign ) * ahv_bbl_ad(ji,jj)
               ...
               !
               zsign             = SIGN(  0.5, - zgdrho * REAL( mgrhu(ji,jj) )  )    ! sign of ( i-gradient * i-slope )
               ahu_bbl_0_ad(ji,jj) = ( 0.5 - zsign ) * ahu_bbl_ad(ji,jj)             ! masked diffusive flux coeff.
   ...
   SUBROUTINE tra_bbl_adj_tst( kumadt )
      ...
      CALL wrk_alloc( jpi, jpj, jpk, jpts, ztsa_tlin , ztsa_tlout, ztsb_tlin , &
         ...
         &                                  zrts )
      CALL wrk_alloc( jpi, jpj, zutr_tlin , zutr_tlout, zvtr_tlin , zvtr_tlout, &
         &                      zutr_adout, zutr_adin , zvtr_adout, zvtr_adin , &
         &                      zr2 )
      ...
      DO jtst = 1, 2
         ...
         CALL grid_random( zr2(:,:), 'U', 0.0_wp, stdu )
         DO jj = nldj, nlej
            DO ji = nldi, nlei
               zutr_tlin(ji,jj) = zr2(ji,jj)
            END DO
         END DO

         CALL grid_random( zr2(:,:), 'V', 0.0_wp, stdv )
         DO jj = nldj, nlej
            DO ji = nldi, nlei
               zvtr_tlin(ji,jj) = zr2(ji,jj)
            END DO
         END DO

         tsa_tl(:,:,:,:) = ztsa_tlin(:,:,:,:)
         tsb_tl(:,:,:,:) = ztsb_tlin(:,:,:,:)
         utr_bbl_tl(:,:) = zutr_tlin(:,:)
         vtr_bbl_tl(:,:) = zvtr_tlin(:,:)
         ...
         zsp1 = DOT_PRODUCT( ztsa_tlout(:,:,:,jp_tem), ztsa_adin(:,:,:,jp_tem) ) &
            ...
            & + DOT_PRODUCT( zutr_tlout, zutr_adin ) + DOT_PRODUCT( zvtr_tlout, zvtr_adin )
         ...
         ztsa_adout(:,:,:,:) = tsa_ad(:,:,:,:)
         ztsb_adout(:,:,:,:) = tsb_ad(:,:,:,:)
         zutr_adout(:,:)     = utr_bbl_ad(:,:)
         zvtr_adout(:,:)     = vtr_bbl_ad(:,:)
         ...
         zsp2 = DOT_PRODUCT( ztsa_tlin(:,:,:,jp_tem), ztsa_adout(:,:,:,jp_tem) ) &
            ...
            & + DOT_PRODUCT( zutr_tlin, zutr_adout ) + DOT_PRODUCT( zvtr_tlin, zvtr_adout )
      ...
      CALL wrk_dealloc( jpi, jpj, jpk, jpts, ztsa_tlin , ztsa_tlout, ztsb_tlin , &
         &                                    ztsa_adout, ztsa_adin , ztsb_adout, &
         &                                    zrts )
      CALL wrk_dealloc( jpi, jpj, zutr_tlin , zutr_tlout, zvtr_tlin , zvtr_tlout, &
         &                        zutr_adout, zutr_adin , zvtr_adout, zvtr_adin , &
         &                        zr2 )
   ...
   !!----------------------------------------------------------------------
CONTAINS
   SUBROUTINE tra_bbl_init_tam               ! Dummy routine
   END SUBROUTINE tra_bbl_init_tam
   SUBROUTINE tra_bbl_tan( kt )              ! Dummy routine
      WRITE(*,*) 'tra_bbl_tan: You should not have seen this print! error?', kt
   END SUBROUTINE tra_bbl_tan
   SUBROUTINE tra_bbl_adj( kt )              ! Dummy routine
      WRITE(*,*) 'tra_bbl_adj: You should not have seen this print! error?', kt
   END SUBROUTINE tra_bbl_adj
   SUBROUTINE tra_bbl_adj_tst( kt )              ! Dummy routine
      WRITE(*,*) 'tra_bbl_adj_tst: You should not have seen this print! error?', kt
   END SUBROUTINE tra_bbl_adj_tst
#endif
   !!======================================================================
