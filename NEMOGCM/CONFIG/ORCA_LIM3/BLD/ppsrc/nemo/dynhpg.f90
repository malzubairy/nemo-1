...
! z- or s-coordinate (1D or 3D + no time dependency) use reference in all cases
   ...
   !!----------------------------------------------------------------------
   !! NEMO/OPA 3.3 , NEMO Consortium (2010)
   !! $Id: dynhpg.F90 3598 2012-11-19 13:35:09Z rblod $
   !! Software governed by the CeCILL licence     (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------
CONTAINS
   ...
   SUBROUTINE hpg_sco( kt )
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1   ! vector opt.
            ...
            zhpi(ji,jj,1) = zcoef0 / e1u(ji,jj) * ( e3w(ji+1,jj  ,1) * ( znad + rhd(ji+1,jj  ,1) )   &
               &                                  - e3w(ji  ,jj  ,1) * ( znad + rhd(ji  ,jj  ,1) ) )
            zhpj(ji,jj,1) = zcoef0 / e2v(ji,jj) * ( e3w(ji  ,jj+1,1) * ( znad + rhd(ji  ,jj+1,1) )   &
               &                                  - e3w(ji  ,jj  ,1) * ( znad + rhd(ji  ,jj  ,1) ) )
      ...
      DO jk = 2, jpkm1
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               dzz  (ji,jj,jk) = gdep3w(ji  ,jj  ,jk) - gdep3w(ji,jj,jk-1)
               drhox(ji,jj,jk) = rhd   (ji+1,jj  ,jk) - rhd   (ji,jj,jk  )
               dzx  (ji,jj,jk) = gdep3w(ji+1,jj  ,jk) - gdep3w(ji,jj,jk  )
               drhoy(ji,jj,jk) = rhd   (ji  ,jj+1,jk) - rhd   (ji,jj,jk  )
               dzy  (ji,jj,jk) = gdep3w(ji  ,jj+1,jk) - gdep3w(ji,jj,jk  )
            END DO
         END DO
      END DO
...
!          true if de3w is really defined as the sum of the e3w scale factors as, it seems to me, it should be
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1   ! vector opt.
            rho_k(ji,jj,1) = -grav * ( e3w(ji,jj,1) - gdep3w(ji,jj,1) )               &
               &                   * (  rhd(ji,jj,1)                                    &
               &                     + 0.5_wp * ( rhd(ji,jj,2) - rhd(ji,jj,1) )         &
               &                              * ( e3w(ji,jj,1) - gdep3w(ji,jj,1) )   &
               &                              / ( gdep3w(ji,jj,2) - gdep3w(ji,jj,1) )  )
         END DO
      END DO
...
!!bug gm    : optimisation: 1/10 and 1/12 the division should be done before the loop
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               rho_k(ji,jj,jk) = zcoef0 * ( rhd   (ji,jj,jk) + rhd   (ji,jj,jk-1) )                                   &
                  &                     * ( gdep3w(ji,jj,jk) - gdep3w(ji,jj,jk-1) )                                   &
                  &            - grav * z1_10 * (                                                                     &
                  &     ( drhow (ji,jj,jk) - drhow (ji,jj,jk-1) )                                                     &
                  &   * ( gdep3w(ji,jj,jk) - gdep3w(ji,jj,jk-1) - z1_12 * ( dzw  (ji,jj,jk) + dzw  (ji,jj,jk-1) ) )   &
               ...
               rho_i(ji,jj,jk) = zcoef0 * ( rhd   (ji+1,jj,jk) + rhd   (ji,jj,jk) )                                   &
                  &                     * ( gdep3w(ji+1,jj,jk) - gdep3w(ji,jj,jk) )                                   &
                  &            - grav* z1_10 * (                                                                      &
                  &     ( drhou (ji+1,jj,jk) - drhou (ji,jj,jk) )                                                     &
                  &   * ( gdep3w(ji+1,jj,jk) - gdep3w(ji,jj,jk) - z1_12 * ( dzu  (ji+1,jj,jk) + dzu  (ji,jj,jk) ) )   &
               ...
               rho_j(ji,jj,jk) = zcoef0 * ( rhd   (ji,jj+1,jk) + rhd   (ji,jj,jk) )                                   &
                  &                     * ( gdep3w(ji,jj+1,jk) - gdep3w(ji,jj,jk) )                                   &
                  &            - grav* z1_10 * (                                                                      &
                  &     ( drhov (ji,jj+1,jk) - drhov (ji,jj,jk) )                                                     &
                  &   * ( gdep3w(ji,jj+1,jk) - gdep3w(ji,jj,jk) - z1_12 * ( dzv  (ji,jj+1,jk) + dzv  (ji,jj,jk) ) )   &
   ...
   SUBROUTINE hpg_prj( kt )
      ...
      DO jj = 1, jpj
        DO ji = 1, jpi
          ...
          ELSE IF(jk < jpkm1) THEN
             DO jkk = jk+1, jpk
                zrhh(ji,jj,jkk) = interp1(gdep3w(ji,jj,jkk),   gdep3w(ji,jj,jkk-1), &
                                         gdep3w(ji,jj,jkk-2), rhd(ji,jj,jkk-1), rhd(ji,jj,jkk-2))
             END DO
          ENDIF
        END DO
      END DO
      ...
      DO jj = 1, jpj;   DO ji = 1, jpi
          zdept(ji,jj,1) = 0.5_wp * e3w(ji,jj,1) - sshn(ji,jj) * znad
      END DO        ;   END DO

      DO jk = 2, jpk;   DO jj = 1, jpj;   DO ji = 1, jpi
          zdept(ji,jj,jk) = zdept(ji,jj,jk-1) + e3w(ji,jj,jk)
      ...
      DO jj = 2, jpj
        DO ji = 2, jpi
          zrhdt1 = zrhh(ji,jj,1) - interp3(zdept(ji,jj,1),asp(ji,jj,1), &
                                         ...
                                         dsp(ji,jj,1) ) * 0.25_wp * e3w(ji,jj,1)

          ! assuming linear profile across the top half surface layer
          zhpi(ji,jj,1) =  0.5_wp * e3w(ji,jj,1) * zrhdt1
        END DO
      END DO
      ...
      DO jj = 2, jpjm1
        DO ji = 2, jpim1
          zu(ji,jj,1) = - ( e3u(ji,jj,1) - sshu_n(ji,jj) * znad)
          zv(ji,jj,1) = - ( e3v(ji,jj,1) - sshv_n(ji,jj) * znad)
        END DO
      END DO
      ...
      DO jk = 2, jpkm1
        DO jj = 2, jpjm1
          DO ji = 2, jpim1
            zu(ji,jj,jk) = zu(ji,jj,jk-1)- e3u(ji,jj,jk)
            zv(ji,jj,jk) = zv(ji,jj,jk-1)- e3v(ji,jj,jk)
          END DO
        END DO
      END DO
      ...
      DO jk = 1, jpkm1
        DO jj = 2, jpjm1
          DO ji = 2, jpim1
            zu(ji,jj,jk) = zu(ji,jj,jk) + 0.5_wp * e3u(ji,jj,jk)
            zv(ji,jj,jk) = zv(ji,jj,jk) + 0.5_wp * e3v(ji,jj,jk)
          END DO
        END DO
      END DO
