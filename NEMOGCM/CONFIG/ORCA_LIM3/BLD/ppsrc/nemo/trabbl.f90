...
CONTAINS
   ...
   SUBROUTINE tra_bbl_init
      ...
      DO jj = 1, jpjm1
         DO ji = 1, jpim1
            mgrhu(ji,jj) = INT(  SIGN( 1.e0, gdept(ji+1,jj,mbkt(ji+1,jj)) - gdept(ji,jj,mbkt(ji,jj)) )  )
            mgrhv(ji,jj) = INT(  SIGN( 1.e0, gdept(ji,jj+1,mbkt(ji,jj+1)) - gdept(ji,jj,mbkt(ji,jj)) )  )
         END DO
      END DO

      DO jj = 1, jpjm1              !* bbl thickness at u- (v-) point
         DO ji = 1, jpim1                 ! minimum of top & bottom e3u_0 (e3v_0)
            e3u_bbl_0(ji,jj) = MIN( e3u(ji,jj,mbkt(ji+1,jj  )), e3u(ji,jj,mbkt(ji,jj)) )
            e3v_bbl_0(ji,jj) = MIN( e3v(ji,jj,mbkt(ji  ,jj+1)), e3v(ji,jj,mbkt(ji,jj)) )
         END DO
      END DO
