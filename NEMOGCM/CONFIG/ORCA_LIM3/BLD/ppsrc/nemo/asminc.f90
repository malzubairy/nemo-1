            ...
            DO jk = 1, jpkm1
               ...
               DO jj = 2, jpjm1
                  DO ji = 2, jpim1   ! vector opt.
                     hdiv(ji,jj) =   &
                        (  e2u(ji  ,jj  ) * e3u(ji  ,jj  ,jk) * u_bkginc(ji  ,jj  ,jk)     &
                         - e2u(ji-1,jj  ) * e3u(ji-1,jj  ,jk) * u_bkginc(ji-1,jj  ,jk)     &
                         + e1v(ji  ,jj  ) * e3v(ji  ,jj  ,jk) * v_bkginc(ji  ,jj  ,jk)     &
                         - e1v(ji  ,jj-1) * e3v(ji  ,jj-1,jk) * v_bkginc(ji  ,jj-1,jk)  )  &
           ...
           DO jj = 1, jpj
              DO ji = 1, jpk
                 fzptnz (ji,jj,jk) = ( -0.0575_wp + 1.710523e-3_wp * SQRT( tsn(ji,jj,jk,jp_sal) )                   & 
                                                  ...
                                                  - 7.53e-4_wp * gdepw(ji,jj,jk)       ! (pressure in dbar) 
              END DO
           END DO
        END DO
