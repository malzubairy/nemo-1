      ...
      DO jk = 2, jpkm1                            !* Slopes at u and v points
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               zbu = MIN(  zbu, -100._wp* ABS( zau ) , -7.e+3_wp/e3u(ji,jj,jk)* ABS( zau )  )
               zbv = MIN(  zbv, -100._wp* ABS( zav ) , -7.e+3_wp/e3v(ji,jj,jk)* ABS( zav )  )
               ...
               zwz(ji,jj,jk) = ( ( 1. - zfi) * zau / ( zbu - zeps )                                              &
                  &                   + zfi  * uslpml(ji,jj)                                                     &
                  &                          * 0.5_wp * ( gdept(ji+1,jj,jk)+gdept(ji,jj,jk)-e3u(ji,jj,1) )   &
                  &                          / MAX( hmlpt(ji,jj), hmlpt(ji+1,jj), 5._wp ) ) * umask(ji,jj,jk)
               zww(ji,jj,jk) = ( ( 1. - zfj) * zav / ( zbv - zeps )                                              &
                  &                   + zfj  * vslpml(ji,jj)                                                     &
                  &                          * 0.5_wp * ( gdept(ji,jj+1,jk)+gdept(ji,jj,jk)-e3v(ji,jj,1) )   &
...
!               zci = 0.5 * ( gdept(ji+1,jj,jk)+gdept(ji,jj,jk) ) / MAX( hmlpt(ji,jj), hmlpt(ji+1,jj), 10. ) )
!               zcj = 0.5 * ( gdept(ji,jj+1,jk)+gdept(ji,jj,jk) ) / MAX( hmlpt(ji,jj), hmlpt(ji,jj+1), 10. ) )
      ...
      DO jk = 2, jpkm1
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               !                                        ! bound the slopes: abs(zw.)<= 1/100 and zb..<0.
               !                                        ! + kxz max= ah slope max =< e1 e3 /(pi**2 2 dt)
               zbi = MIN( zbw ,- 100._wp* ABS( zai ) , -7.e+3_wp/e3w(ji,jj,jk)* ABS( zai )  )
               zbj = MIN( zbw , -100._wp* ABS( zaj ) , -7.e+3_wp/e3w(ji,jj,jk)* ABS( zaj )  )
               ...
               zck = gdepw(ji,jj,jk) / MAX( hmlp(ji,jj), 10._wp )
   ...
   SUBROUTINE ldf_slp_grif ( kt )
            ...
            DO jj = 1, jpj                    ! NB: not masked ==>  a minimum value is set
               DO ji = 1, jpi                 ! vector opt.
                  ...
                  ELSE
                     ...
                     zdks = 0._wp
                  ENDIF
                  zdzrho_raw = ( - zalbet(ji   ,jj   ,jk) * zdkt + zbeta0*zdks ) / e3w(ji,jj,jk+kp)
      ...
      DO jj = 1, jpj                          !==  Reciprocal depth of the w-point below ML base  ==!
         DO ji = 1, jpi
            ...
            z1_mlbw(ji,jj) = 1._wp / gdepw(ji,jj,jk)
         END DO
      END DO
      !
            ...
            DO jj = 1, jpjm1
               DO ji = 1, jpim1
                  ...
                  zti_g_raw = (  zdxrho(ji+ip,jj,jk-kp,1-ip) / zdzrho(ji+ip,jj,jk-kp,kp)      &
                     &      - ( gdept(ji+1,jj,jk-kp) - gdept(ji,jj,jk-kp) ) / e1u(ji,jj)  ) * umask(ji,jj,jk)
                  jk = MIN( nmln(ji,jj+jp) , mbkt(ji,jj+jp) ) + 1
                  ztj_g_raw = (  zdyrho(ji,jj+jp,jk-kp,1-jp) / zdzrho(ji,jj+jp,jk-kp,kp)      &
                     &      - ( gdept(ji,jj+1,jk-kp) - gdept(ji,jj,jk-kp) ) / e2v(ji,jj)  ) * vmask(ji,jj,jk)
            ...
            DO jk = 1, jpkm1
               ...
               DO jj = 1, jpjm1
                  DO ji = 1, jpim1         ! vector opt.
                     ...
                     zti_coord = znot_thru_surface * ( gdept(ji+1,jj  ,jk) - gdept(ji,jj,jk) ) / e1u(ji,jj)
                     ztj_coord = znot_thru_surface * ( gdept(ji  ,jj+1,jk) - gdept(ji,jj,jk) ) / e2v(ji,jj)                  ! unmasked
                     ...
                     zti_g_lim =          ( zfacti   * zti_g_lim                       &
                        ...
                        &                           * gdepw(ji+ip,jj,jk+kp) * z1_mlbw(ji+ip,jj) ) * umask(ji,jj,jk+kp)
                     ztj_g_lim =          ( zfactj   * ztj_g_lim                       &
                        &      + ( 1._wp - zfactj ) * ztj_mlb(ji,jj+jp,1-jp,kp)   &
                        &                           * gdepw(ji,jj+jp,jk+kp) * z1_mlbw(ji,jj+jp) ) * vmask(ji,jj,jk+kp)
                     !
                    ...
                    !
                     ...
                     zbti = e1t(ji+ip,jj) * e2t(ji+ip,jj) * e3w(ji+ip,jj,jk+kp)
                     zbtj = e1t(ji,jj+jp) * e2t(ji,jj+jp) * e3w(ji,jj+jp,jk+kp)
                     !
   ...
   SUBROUTINE ldf_slp_mxl( prd, pn2, p_gru, p_grv, p_dzr )
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1
            ...
            !                        !- bound the slopes: abs(zw.)<= 1/100 and zb..<0
            !                           kxz max= ah slope max =< e1 e3 /(pi**2 2 dt)
            zbu = MIN(  zbu , -100._wp* ABS( zau ) , -7.e+3_wp/e3u(ji,jj,iku)* ABS( zau )  )
            zbv = MIN(  zbv , -100._wp* ABS( zav ) , -7.e+3_wp/e3v(ji,jj,ikv)* ABS( zav )  )
            ...
            !                        !- bound the slopes: abs(zw.)<= 1/100 and zb..<0.
            !                           kxz max= ah slope max =< e1 e3 /(pi**2 2 dt)
            zbi = MIN(  zbw , -100._wp* ABS( zai ) , -7.e+3_wp/e3w(ji,jj,ik)* ABS( zai )  )
            zbj = MIN(  zbw , -100._wp* ABS( zaj ) , -7.e+3_wp/e3w(ji,jj,ik)* ABS( zaj )  )
      ...
      ELSE                             ! Madec operator : slopes at u-, v-, and w-points
         ...
         IF( (ln_traldf_hor .OR. ln_dynldf_hor) .AND. .NOT. (lk_vvl .AND. ln_rstart) ) THEN
            ...
            DO jk = 1, jpk
               DO jj = 2, jpjm1
                  DO ji = 2, jpim1   ! vector opt.
                     uslp (ji,jj,jk) = -1./e1u(ji,jj) * ( gdept(ji+1,jj,jk) - gdept(ji ,jj ,jk) ) * umask(ji,jj,jk)
                     vslp (ji,jj,jk) = -1./e2v(ji,jj) * ( gdept(ji,jj+1,jk) - gdept(ji ,jj ,jk) ) * vmask(ji,jj,jk)
                     wslpi(ji,jj,jk) = -1./e1t(ji,jj) * ( gdepw(ji+1,jj,jk) - gdepw(ji-1,jj,jk) ) * tmask(ji,jj,jk) * 0.5
                     wslpj(ji,jj,jk) = -1./e2t(ji,jj) * ( gdepw(ji,jj+1,jk) - gdepw(ji,jj-1,jk) ) * tmask(ji,jj,jk) * 0.5
                  END DO
               END DO
            END DO
