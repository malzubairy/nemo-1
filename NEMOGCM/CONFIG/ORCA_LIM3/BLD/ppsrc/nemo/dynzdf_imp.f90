   ...
   SUBROUTINE dyn_zdf_imp( kt, p2dt )
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1
            ...
            avmu(ji,jj,ikbu+1) = -bfrua(ji,jj) * e3uw(ji,jj,ikbu+1) 
            avmv(ji,jj,ikbv+1) = -bfrva(ji,jj) * e3vw(ji,jj,ikbv+1)
         END DO
      END DO
      ENDIF
      ...
      DO jk = 1, jpkm1        ! Matrix
         DO jj = 2, jpjm1 
            DO ji = 2, jpim1   ! vector opt.
               zcoef = - p2dt / e3u(ji,jj,jk)
               zzwi          = zcoef * avmu (ji,jj,jk  ) / e3uw(ji,jj,jk  )
               zwi(ji,jj,jk) = zzwi  * umask(ji,jj,jk)
               zzws          = zcoef * avmu (ji,jj,jk+1) / e3uw(ji,jj,jk+1)
      ...
      DO jk = 1, jpkm1        ! Matrix
         DO jj = 2, jpjm1   
            DO ji = 2, jpim1   ! vector opt.
               zcoef = -p2dt / e3v(ji,jj,jk)
               zzwi          = zcoef * avmv (ji,jj,jk  ) / e3vw(ji,jj,jk  )
               zwi(ji,jj,jk) =  zzwi * vmask(ji,jj,jk)
               zzws          = zcoef * avmv (ji,jj,jk+1) / e3vw(ji,jj,jk+1)
