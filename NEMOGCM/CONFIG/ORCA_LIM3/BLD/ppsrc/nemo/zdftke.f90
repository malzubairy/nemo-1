...
!!bfr   - end commented area
      ...
      IF( ln_lc ) THEN      !  Langmuir circulation source term added to tke       (Axell JGR 2002)
         ...
         zpelc(:,:,1) =  MAX( rn2b(:,:,1), 0._wp ) * gdepw(:,:,1) * e3w(:,:,1)
         DO jk = 2, jpk
            zpelc(:,:,jk)  = zpelc(:,:,jk-1) + MAX( rn2b(:,:,jk), 0._wp ) * gdepw(:,:,jk) * e3w(:,:,jk)
         END DO
...
!CDIR NOVERRCHK
               DO ji = 2, jpim1   ! vector opt.
                  ...
                  en(ji,jj,jk) = en(ji,jj,jk) + zbbrau * zdif * EXP( -gdepw(ji,jj,jk) / htau(ji,jj) )   &
                     &                                        * ( 1._wp - fr_i(ji,jj) ) * tmask(ji,jj,jk)
               END DO
            END DO
         END DO
...
!CDIR NOVERRCHK
      ...
      CASE ( 0 )           ! bounded by the distance to surface and bottom
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zemxl = MIN( gdepw(ji,jj,jk), zmxlm(ji,jj,jk),   &
                  &            gdepw(ji,jj,mbkt(ji,jj)+1) - gdepw(ji,jj,jk) )
      ...
      CASE ( 1 )           ! bounded by the vertical scale factor
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zemxl = MIN( e3w(ji,jj,jk), zmxlm(ji,jj,jk) )
                  zmxlm(ji,jj,jk) = zemxl
                  zmxld(ji,jj,jk) = zemxl
               END DO
            END DO
      ...
      CASE ( 2 )           ! |dk[xml]| bounded by e3t :
         ...
         DO jk = jpkm1, 2, -1     ! from the bottom to the surface :
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zemxl = MIN( zmxlm(ji,jj,jk+1) + e3t(ji,jj,jk+1), zmxlm(ji,jj,jk) )
                  zmxlm(ji,jj,jk) = zemxl
                  zmxld(ji,jj,jk) = zemxl
               END DO
            END DO
