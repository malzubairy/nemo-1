...
9020  FORMAT(I5,11D15.7)
   ...
   SUBROUTINE dia_hsb_init
      ...
      DO jk = 1, jpk
         e3t_ini   (:,:,jk) = e3t(:,:,jk)                        ! initial vertical scale factors
         hc_loc_ini(:,:,jk) = tsn(:,:,jk,jp_tem) * e3t(:,:,jk)   ! initial heat content
         sc_loc_ini(:,:,jk) = tsn(:,:,jk,jp_sal) * e3t(:,:,jk)   ! initial salt content
      END DO
