...
CONTAINS
   ...
   SUBROUTINE sbc_rnf_div( phdivn )
      ...
      ELSE                       !==   runoff put only at the surface   ==!
         ...
         ENDIF
         phdivn(:,:,1) = phdivn(:,:,1) - ( rnf(:,:) + rnf_b(:,:) ) * zfact * r1_rau0 / e3t(:,:,1)
      ENDIF
      !
         ...
         IF( ln_rnf_depth ) THEN                    ! depth of runoffs set from a file
            ...
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( h_rnf(ji,jj) > 0._wp ) THEN
                     jk = 2
                     DO WHILE ( jk /= mbkt(ji,jj) .AND. gdept(ji,jj,jk) < h_rnf(ji,jj) ) ;  jk = jk + 1 ;  END DO
                     nk_rnf(ji,jj) = jk
            ...
            DO jj = 1, jpj                                ! set the associated depth
               DO ji = 1, jpi
                  h_rnf(ji,jj) = 0._wp
                  DO jk = 1, nk_rnf(ji,jj)
                     h_rnf(ji,jj) = h_rnf(ji,jj) + e3t(ji,jj,jk)
                  END DO
               END DO
            END DO
