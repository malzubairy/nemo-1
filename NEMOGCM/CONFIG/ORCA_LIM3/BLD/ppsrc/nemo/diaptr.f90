...
!!gm here, use of tmask_i  ==> no need of loop over nldi, nlei....
               DO ji =  nldi, nlei   ! No vector optimisation here. Better use a mask ?
                  p_fval(jj,jk) = p_fval(jj,jk) + pva(ji,jj,jk) * e1v(ji,jj) * e3v(ji,jj,jk) * pmsk(ji,jj)
               END DO
            END DO
         END DO
      ELSE 
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji =  nldi, nlei   ! No vector optimisation here. Better use a mask ?
                  p_fval(jj,jk) = p_fval(jj,jk) + pva(ji,jj,jk) * e1v(ji,jj) * e3v(ji,jj,jk) * tmask_i(ji,jj)
               END DO
            END DO
         END DO
      END IF
   ...
   FUNCTION ptr_tjk( pta, pmsk )   RESULT ( p_fval )
      ...
      DO jk = 1, jpkm1
         DO jj = 2, jpjm1
            DO ji =  nldi, nlei   ! No vector optimisation here. Better use a mask ?
               p_fval(jj,jk) = p_fval(jj,jk) + pta(ji,jj,jk) * e1t(ji,jj) * e3t(ji,jj,jk) * pmsk(ji,jj)
            END DO
         END DO
      END DO
      !
