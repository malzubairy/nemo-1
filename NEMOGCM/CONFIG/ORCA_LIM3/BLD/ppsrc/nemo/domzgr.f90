...
CONTAINS       
   ...
   SUBROUTINE dom_zgr
      ...
      IF( nprint == 1 .AND. lwp )   THEN
         ...
         WRITE(numout,*) ' MIN val depth t ', MINVAL( gdept(:,:,:) ),   &
            &                   ' w ',   MINVAL( gdepw(:,:,:) ), '3w ', MINVAL( gdep3w(:,:,:) )
         WRITE(numout,*) ' MIN val e3    t ', MINVAL( e3t(:,:,:) ), ' f ', MINVAL( e3f(:,:,:) ),  &
            &                   ' u ',   MINVAL( e3u(:,:,:) ), ' u ', MINVAL( e3v(:,:,:) ),  &
            &                   ' uw',   MINVAL( e3uw(:,:,:)), ' vw', MINVAL( e3vw(:,:,:)),   &
            &                   ' w ',   MINVAL( e3w(:,:,:) )

         WRITE(numout,*) ' MAX val depth t ', MAXVAL( gdept(:,:,:) ),   &
            &                   ' w ',   MAXVAL( gdepw(:,:,:) ), '3w ', MAXVAL( gdep3w(:,:,:) )
         WRITE(numout,*) ' MAX val e3    t ', MAXVAL( e3t(:,:,:) ), ' f ', MAXVAL( e3f(:,:,:) ),  &
            &                   ' u ',   MAXVAL( e3u(:,:,:) ), ' u ', MAXVAL( e3v(:,:,:) ),  &
            &                   ' uw',   MAXVAL( e3uw(:,:,:)), ' vw', MAXVAL( e3vw(:,:,:)),   &
            &                   ' w ',   MAXVAL( e3w(:,:,:) )
      ENDIF
      !
...
!!
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            DO jk = 1, jpkm1
               IF( scobot(ji,jj) >= gdept(ji,jj,jk) )   mbathy(ji,jj) = MAX( 2, jk )
      ...
      IF(lwp) THEN                                  ! selected vertical profiles
         ...
         WRITE(numout,"(10x,i4,4f9.2)") ( jk, gdept(1,1,jk), gdepw(1,1,jk),     &
            &                                 e3t(1,1,jk), e3w(1,1,jk), jk=1,jpk )
         DO jj = mj0(20), mj1(20)
            DO ji = mi0(20), mi1(20)
               WRITE(numout,*)
               ...
               WRITE(numout,*) ' ~~~~~~  --------------------'
               WRITE(numout,"(9x,' level   gdept    gdepw    gde3w     e3t      e3w  ')")
               WRITE(numout,"(10x,i4,4f9.2)") ( jk, gdept(ji,jj,jk), gdepw(ji,jj,jk),     &
                  &                                 e3t(ji,jj,jk), e3w(ji,jj,jk), jk=1,jpk )
            END DO
         END DO
         DO jj = mj0(74), mj1(74)
...
!!gm bug?  no more necessary?  if ! defined key_helsinki
      DO jk = 1, jpk
         DO jj = 1, jpj
            DO ji = 1, jpi
               IF( e3w(ji,jj,jk) <= 0._wp .OR. e3t(ji,jj,jk) <= 0._wp ) THEN
                  WRITE(ctmp1,*) 'zgr_sco :   e3w   or e3t   =< 0  at point (i,j,k)= ', ji, jj, jk
                  CALL ctl_stop( ctmp1 )
               ENDIF
               IF( gdepw(ji,jj,jk) < 0._wp .OR. gdept(ji,jj,jk) < 0._wp ) THEN
