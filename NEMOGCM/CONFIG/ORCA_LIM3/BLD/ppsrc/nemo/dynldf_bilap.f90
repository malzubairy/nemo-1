...
!!$      zlv(:,:,jpk) = 0.e0
      ...
      DO jk = 1, jpkm1                                 ! Horizontal slab
         ...
         IF( ln_sco .OR. ln_zps ) THEN   ! s-coordinate or z-coordinate with partial steps
            zuf(:,:,jk) = rotb(:,:,jk) * e3f(:,:,jk)
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zlu(ji,jj,jk) = - ( zuf(ji,jj,jk) - zuf(ji,jj-1,jk) ) / ( e2u(ji,jj) * e3u(ji,jj,jk) )   &
                     &         + ( hdivb(ji+1,jj,jk) - hdivb(ji,jj,jk) ) / e1u(ji,jj)
   
                  zlv(ji,jj,jk) = + ( zuf(ji,jj,jk) - zuf(ji-1,jj,jk) ) / ( e1v(ji,jj) * e3v(ji,jj,jk) )   &
      ...
      DO jk = 1, jpkm1
         ...
         zlu(:,:,jk) = zlu(:,:,jk) * ahm3(:,:,jk)
         zlv(:,:,jk) = zlv(:,:,jk) * ahm4(:,:,jk)
         ...
         DO jj = 1, jpjm1
            DO ji = 1, jpim1   ! vector opt.
               zlu(ji,jj,jk) = e2u(ji,jj) * e3u(ji,jj,jk) * zlu(ji,jj,jk)
               zlv(ji,jj,jk) = e1v(ji,jj) * e3v(ji,jj,jk) * zlv(ji,jj,jk)
            END DO
         END DO
