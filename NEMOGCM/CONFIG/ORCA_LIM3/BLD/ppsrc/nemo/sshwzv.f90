...
CONTAINS
   ...
   SUBROUTINE ssh_wzv( kt ) 
      ...
      IF( lk_vvl ) THEN                           !  Regridding: Update Now Vertical coord.  !   (only in vvl case)
         !                                        !------------------------------------------!
         DO jk = 1, jpkm1
            gdept(:,:,jk) = gdept(:,:,jk)         ! now local depths stored in fsdep. arrays
            gdepw(:,:,jk) = gdepw(:,:,jk)
            gdep3w(:,:,jk) = gdep3w(:,:,jk)
            !
            e3t(:,:,jk) = e3t(:,:,jk)         ! vertical scale factors stored in fse3. arrays
            e3u(:,:,jk) = e3u(:,:,jk)
            e3v(:,:,jk) = e3v(:,:,jk)
            e3f(:,:,jk) = e3f(:,:,jk)
            e3w(:,:,jk) = e3w(:,:,jk)
